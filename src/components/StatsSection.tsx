...
export default function StatsSection() {
  ...
  return (
    <section id="experience-stats" className="py-16 bg-slate-900/40 border-y border-slate-800/80">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`p-6 rounded-2xl bg-gradient-to-br ${stat.bg} border border-slate-800/80 hover:border-slate-700 transition-all group`}
              >
                <div className="flex items-center justify-between mb-4">
                  <IconComponent className={`w-6 h-6 ${stat.color} group-hover:scale-110 transition-transform`} />
                  <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Metric</span>
                </div>
