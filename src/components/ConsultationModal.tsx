...
export default function ConsultationModal({ isOpen, onClose, initialService }: ConsultationModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
        
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-full bg-slate-950 border border-slate-800 text-slate-400 hover:text-white transition-all cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        {submitted ? (
          <div className="py-6 text-center space-y-4">
            ...
            <p className="text-xs text-slate-300">
              Hariom will review your requirements and reply to your email within 24 hours.
            </p>
            <div className="pt-2">
              <button
                onClick={onClose}
                className="px-6 py-2.5 rounded-xl bg-blue-600 text-white font-semibold text-xs cursor-pointer"
              >
                Close Window
              </button>
            </div>
          ...
          <div>
            <div className="mb-6">
              <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-blue-950 text-blue-400 text-[11px] font-mono border border-blue-800/50 mb-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-mono text-slate-300 mb-1">Service Required</label>
                <select
              ...
              <div>
                <label className="block text-[11px] font-mono text-slate-300 mb-1">Message / Requirements</label>
                <textarea
                  rows={3}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs shadow-lg shadow-blue-600/25 flex items-center justify-center space-x-2 cursor-pointer"
              >
            ...
            <div className="mt-4 pt-4 border-t border-slate-800 text-center">
              <span className="text-[11px] text-slate-400">Or chat directly:</span>
              <a
                href={whatsappUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-2 w-full py-2.5 px-4 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 font-semibold text-xs flex items-center justify-center space-x-2 cursor-pointer transition-all"
              >
